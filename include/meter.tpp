...
template <typename T>
double calcualteMean (const std::vector<T> & x) {
  if (x.size() == 0) 
...
// 		      , const std::string & format
  ...
  if (! std::is_arithmetic <T>::value)
    throw std::invalid_argument ("Error: Cannot calculate statistics of measurements of non arithmetic type.");  
  ...
  for (uint i = 0; i < measurements.rows(); i++) {
    std::vector <T> measurement (measurements.getRow (i));
    
    double min = (*std::min_element (measurement.begin(), measurement.end()));
    double max = (*std::max_element (measurement.begin(), measurement.end()));
    
    double mean = calcualteMean (measurement);
    double sd   = std::sqrt (calcualteMean (measurement));
